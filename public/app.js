...
function showError(message){
 ...
 const support=document.createElement('span');support.className='error-support';
 support.append(document.createTextNode('Sorun devam ederse bize yaz: '));
 const link=document.createElement('a');link.href='mailto:remziazakli@gmail.com?subject=ARVUS%20ba%C5%9Fvuru%20deste%C4%9Fi';link.textContent='remziazakli@gmail.com';
 support.append(link);error.append(support);error.hidden=false;
}
function syncCustomArea(){const field=form.elements.customArea;const visible=selected==='other'&&step===0;document.querySelector('#custom-area-wrap').hidden=!visible;field.required=selected==='other';if(selected!=='other'){field.value='';field.removeAttribute('aria-invalid');}}
function renderStep(focus=true){next.disabled=step===0&&!form.elements.category.value;document.querySelectorAll('.category[data-category]').forEach(a=>a.classList.toggle('is-selected',a.dataset.category===form.elements.category.value));document.querySelector('#chosen-field').hidden=step===0||!selected;if(selected)document.querySelector('#chosen-field-name').textContent=categories[selected].name;syncCustomArea();panels.forEach((p,i)=>p.hidden=i!==step);document.querySelectorAll('.steps li').forEach((li,i)=>{li.classList.toggle('active',i===step);li.classList.toggle('done',i<step);if(i===step)li.setAttribute('aria-current','step');else li.removeAttribute('aria-current');});back.hidden=step===0;document.querySelector('#footer-caption').hidden=step!==0;next.replaceChildren(document.createTextNode(step===3?'Başvuruyu gönder':'Devam et'));const arrow=document.createElement('span');arrow.textContent='↗';arrow.setAttribute('aria-hidden','true');next.append(arrow);error.hidden=true;if(focus){const heading=panels[step].querySelector('h2');heading.tabIndex=-1;heading.focus({preventScroll:true});document.querySelector('#application').scrollIntoView({behavior:'smooth',block:'start'});}}
...
function renderReview(){document.querySelector('#review-category').textContent=categories[selected].name;const rows=[['Ad soyad',value('fullName')],['Telefon',value('phone')],['E-posta',value('email')],['Başvuru alanı',selected==='other'?value('customArea'):categories[selected].name],['Üniversite',value('university')],['Bölüm / sınıf',value('department')+' · '+value('year')],['Portfolyo',value('portfolio')||'Belirtilmedi'],['İlgi alanları',(skillSelections[selected]||[]).join(', ')||'Belirtilmedi'],['Deneyim',value('experience')],['Haftalık süre',value('time')],['Motivasyon',value('motivation')],['Proje / fikir',value('project')||'Belirtilmedi']];const target=document.querySelector('#review');target.replaceChildren();rows.forEach(([label,text])=>{const row=document.createElement('div');row.className='review-row';const dt=document.createElement('dt');const dd=document.createElement('dd');dt.textContent=label;dd.textContent=text;row.append(dt,dd);target.append(row);});}
...
form.addEventListener('submit',async event=>{event.preventDefault();if(submitting||!validate())return;if(step===0){const choice=form.elements.category.value;if(selected!==choice){selected=choice;renderSkills();}}if(step===2)renderReview();if(step<3){step++;renderStep();}else{
 ...
 for(const name of ['fullName','phone','email','customArea','university','department','year','portfolio','experience','time','motivation','project'])payload[name]=value(name);
 let timer;
 try {
...
document.querySelectorAll('.category[data-category="other"]').forEach(card=>card.addEventListener('click',event=>{
 event.preventDefault();selected='other';form.elements.category.value='other';renderSkills();step=0;renderStep();
 requestAnimationFrame(()=>document.querySelector('#application').scrollIntoView({behavior:'smooth',block:'start'}));
}));
